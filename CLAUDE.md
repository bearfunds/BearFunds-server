...
- **Operator runs commands in PowerShell (Windows), not bash.** Write every operator-facing hand-off command (git, npm, supabase, tests, curl, file ops) in PowerShell syntax (`Copy-Item`/`copy` not `cp`, `$env:VAR=...` not `export`, backtick line-continuation, `;`/newlines not `&&`, `Invoke-RestMethod`/`curl.exe` not bare `curl`). The Linux-sandbox commands Claude runs itself stay bash.
- **Coverage ratchet.** A bug fix lands with the regression test that would have caught it; a feature or contract/tenancy change lands with new or extended tests (incl. an RLS-isolation test for any tenant-table change); a green claim names its run or says "not run"; and the coverage question is answered explicitly before the work is "done". (Canonical rules: brain `CLAUDE.md` "Coverage ratchet"; how-to in the brain Testing Methodology.)
...
The **BearFunds brain vault** owns the decisions, the maps, and the decision trail. This repo owns the runtime and the canonical Schema Contract. When a "why" is needed, it lives in the brain.
...
4. **The desktop Read is authoritative over the sandbox view** (stale-mount phantoms are documented); never `npm install` onto a Windows-mounted repo from the sandbox.
